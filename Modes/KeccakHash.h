...
/**
  ...
  * output bits if the length was specified when calling
 * Keccak_HashInitialize().
  * @param  hashInstance    Pointer to the hash instance initialized by
 * Keccak_HashInitialize().
  * If @a hashbitlen was not 0 in the call to Keccak_HashInitialize(), the
 * number of
  *     output bits is equal to @a hashbitlen.
  * If @a hashbitlen was 0 in the call to Keccak_HashInitialize(), the output
 * bits
  *     must be extracted using the Keccak_HashSqueeze() function.
  * @param  state       Pointer to the state of the sponge function initialized
 * by Init().
