...
HashReturn Keccak_HashUpdate(Keccak_HashInstance* instance,
  ...
  if ((databitlen % 8) == 0)
    ...
    HashReturn ret =
        Keccak_SpongeAbsorb(&instance->sponge, data, databitlen / 8);
    if (ret == SUCCESS) {
      ...
      unsigned short delimitedLastBytes =
          ...
          ((unsigned short)instance->delimitedSuffix << (databitlen % 8));
...
HashReturn Keccak_HashFinal(Keccak_HashInstance* instance,
                            BitSequence* hashval) {
  HashReturn ret = Keccak_SpongeAbsorbLastFewBits(&instance->sponge,
                                                  instance->delimitedSuffix);
  if (ret == SUCCESS)
    return Keccak_SpongeSqueeze(
...
HashReturn Keccak_HashSqueeze(Keccak_HashInstance* instance,
                              BitSequence* data,
                              DataLength databitlen) {
