...
unsigned int appendSuffixToMessage(char* out,
                                   const char* in,
                                   unsigned int inputLengthInBits,
                                   unsigned char delimitedSuffix) {
  memcpy(out, in, (inputLengthInBits + 7) / 8);
  if (delimitedSuffix == 0x00) abort();
...
void displaySpongeIntermediateValuesOne(const unsigned char* message,
                                        unsigned int messageLength,
                                        unsigned char delimitedSuffix,
                                        unsigned int rate,
                                        unsigned int capacity,
                                        unsigned int outputLengthInBits) {
  ...
  messageLengthWithSuffix = appendSuffixToMessage(
      messageWithSuffix, message, messageLength, delimitedSuffix);
  if (delimitedSuffix != 0x01) {
    ...
    displayBits(2,
                "Suffix (in bits)",
                suffix,
                messageLengthWithSuffix - messageLength,
                0);
    displayBits(2,
                "Input message with suffix appended to it (in bits)",
                messageWithSuffix,
                messageLengthWithSuffix,
                0);
    displayBytes(
        2,
        "Input message with suffix appended to it (last byte aligned on LSB)",
        messageWithSuffix,
        (messageLengthWithSuffix + 7) / 8);
  }
  ...
  if ((messageLengthWithSuffix % 8) != 0)
    Keccak_SpongeAbsorbLastFewBits(
        &sponge,
        messageWithSuffix[messageLengthWithSuffix / 8] |
            (1 << (messageLengthWithSuffix % 8)));
...
void displaySpongeIntermediateValuesFew(
    const char* fileName,
    unsigned char delimitedSuffix,
    unsigned int rate,
    unsigned int capacity,
  ...
  const unsigned char* message30 =
      (unsigned char*)"\x53\x58\x7B\x19";  // 110010100001101011011110100110
  ...
  fprintf(f, "\n");
  displaySpongeIntermediateValuesOne(
      message0, 0, delimitedSuffix, rate, capacity, desiredOutputLengthInBits);

  fprintf(f, "+++ Example with a small message +++\n");
  fprintf(f, "\n");
  fprintf(f,
          "This is the message of length 5 from "
          "http://csrc.nist.gov/groups/ST/toolkit/examples.html .\n");
  fprintf(f, "\n");
  displaySpongeIntermediateValuesOne(
      message5, 5, delimitedSuffix, rate, capacity, desiredOutputLengthInBits);

  fprintf(f, "+++ Example with a small message +++\n");
  fprintf(f, "\n");
  fprintf(f,
          "This is the message of length 30 from "
          "http://csrc.nist.gov/groups/ST/toolkit/examples.html .\n");
  fprintf(f, "\n");
  displaySpongeIntermediateValuesOne(message30,
                                     30,
                                     delimitedSuffix,
                                     rate,
                                     capacity,
                                     desiredOutputLengthInBits);

  fprintf(f, "+++ Example with a larger message +++\n");
  fprintf(f, "\n");
  fprintf(f, "This is the message of length 2008 from ShortMsgKAT.txt.\n");
  fprintf(f, "\n");
  displaySpongeIntermediateValuesOne(message2008,
                                     2008,
                                     delimitedSuffix,
                                     rate,
                                     capacity,
...
void displaySpongeIntermediateValues(void) {
  ...
  displaySpongeIntermediateValuesFew(
      "KeccakSpongeIntermediateValues_SHAKE128.txt", 0x1F, 1344, 256, 4096);
  displaySpongeIntermediateValuesFew(
      "KeccakSpongeIntermediateValues_SHAKE256.txt", 0x1F, 1088, 512, 4096);
  displaySpongeIntermediateValuesFew(
      "KeccakSpongeIntermediateValues_SHA3-224.txt", 0x06, 1152, 448, 224);
  displaySpongeIntermediateValuesFew(
      "KeccakSpongeIntermediateValues_SHA3-256.txt", 0x06, 1088, 512, 256);
  displaySpongeIntermediateValuesFew(
      "KeccakSpongeIntermediateValues_SHA3-384.txt", 0x06, 832, 768, 384);
  displaySpongeIntermediateValuesFew(
      "KeccakSpongeIntermediateValues_SHA3-512.txt", 0x06, 576, 1024, 512);
}

void displayDuplexIntermediateValuesOne(FILE* f,
                                        unsigned int rate,
                                        unsigned int capacity) {
  ...
  for (i = 0; i <= rate + 120; i += 123) {
    ...
    if ((inBitLen % 8) == 0)
      Keccak_Duplexing(
          &duplex, input, inBitLen / 8, output, (outBitLen + 7) / 8, 0x01);
    else
      Keccak_Duplexing(&duplex,
                       input,
                       inBitLen / 8,
                       output,
                       (outBitLen + 7) / 8,
                       input[inBitLen / 8] | (1 << (inBitLen % 8)));
    {
      char text[100];
...
void displayDuplexIntermediateValues() {
  ...
  for (rate = 1026; rate <= 1027; rate++) {
    ...
    sprintf(
        fileName, "KeccakDuplexIntermediateValues_r%dc%d.txt", rate, capacity);
    f = fopen(fileName, "w");
    if (f == NULL)
      printf("Could not open %s\n", fileName);
    else {
