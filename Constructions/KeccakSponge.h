...
ALIGN typedef struct Keccak_SpongeInstanceStruct {
  ...
  unsigned int rate;
  /** The position in the state of the next byte to be input (when absorbing) or
   * output (when squeezing). */
...
/**
  ...
  * @pre    One must have r+c=1600 and the rate a multiple of 8 bits (one byte)
 * in this implementation.
  * @return Zero if successful, 1 otherwise.
  */
int Keccak_SpongeInitialize(Keccak_SpongeInstance* spongeInstance,
                            unsigned int rate,
                            unsigned int capacity);

/**
  * Function to give input data bytes for the sponge function to absorb.
  * @param  spongeInstance  Pointer to the sponge instance initialized by
 * Keccak_SpongeInitialize().
  ...
  * @return Zero if successful, 1 otherwise.
  */
int Keccak_SpongeAbsorb(Keccak_SpongeInstance* spongeInstance,
 ...
 * Keccak_SpongeInitialize().
  ...
  *                     These <i>n</i> bits must be in the least significant bit
 * positions.
 ...
 * <i>n</i>
  ...
  *                     Some examples:
  *                         - If no bits are to be absorbed, then @a
 * delimitedData must be 0x01.
  *                         - If the 2-bit sequence 0,0 is to be absorbed, @a
 * delimitedData must be 0x04.
  *                         - If the 5-bit sequence 0,1,0,0,1 is to be absorbed,
 * @a delimitedData must be 0x32.
  *                         - If the 7-bit sequence 1,1,0,1,0,0,0 is to be
 * absorbed, @a delimitedData must be 0x8B.
  *                     .
  * @pre    The sponge function must be in the absorbing phase,
  *         i.e., Keccak_SpongeSqueeze() or Keccak_SpongeAbsorbLastFewBits()
  *         must not have been called before.
  ...
  * @return Zero if successful, 1 otherwise.
  */
int Keccak_SpongeAbsorbLastFewBits(Keccak_SpongeInstance* spongeInstance,
 ...
 * Keccak_SpongeInitialize().
  ...
  * @return Zero if successful, 1 otherwise.
  */
int Keccak_SpongeSqueeze(Keccak_SpongeInstance* spongeInstance,
                         unsigned char* data,
                         unsigned long long dataByteLen);
